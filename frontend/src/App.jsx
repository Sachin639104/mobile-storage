...
export default function App() {
  ...
  const handlePrint = (receipt, type = 'return') => {
    ...
    const content = type === 'exchange' ? `
      ...
      <p><strong>${diff > 0 ? `Customer Pays Extra: ₹${diff}` : `Refund: ₹${Math.abs(diff)}`}
      </strong></p>
  ...
  const priceDiff = exchangeModal
    ? (mobiles.find(m => m.modelName === exchangeData.newModelName)?.price || 0) - 
    (exchangeModal.totalPaid || 0)
    : 0;
                      ...
                      <tr key={r._id}>
                        ...
                        <td>
                          ...
                          {r.status === 'returned' && (
                            <button
                              className="btn btn-sm px-2 py-1"
                              style={{background:'rgba(68,136,255,0.15)', color:'#4488ff', border:'1px solid rgba(68,136,255,0.3)', 
                                fontSize:'11px'}}
